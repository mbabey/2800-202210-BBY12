...
app.put('/update-users', function(req, res) {
    ...
    connection.query('UPDATE users SET user_name = ? AND first_name = ? AND last_name = ? AND email_address = ? AND password = ? WHERE ID = ?', [req.body.user_name, req.body.first_name, req.body.last_name, req.body.email_address, req.body.password, req.body.id],
        function(error, results, fields) {
            if (error) {
                console.log(error);
            }
...
module.exports = {
    createAccount: function(req, res) {
        ...
        connection.query('INSERT INTO users (username, password, fName, lName, email, phoneNo, location, description) values (?, ?,?,?,?,?,?,?)', [req.body.username, req.body.email],
            function(error, results, fields) {
                if (error) {
                    console.log(error);
                }
