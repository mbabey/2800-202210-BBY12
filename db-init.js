...
async function initDB(con) {
    ...
    await con.query(`
            ...
            USE COMP2800;
            CREATE TABLE IF NOT EXISTS \`BBY12-Users\` (
            ...
            CREATE TABLE IF NOT EXISTS \`BBY12-Post\` (
                ...
                CONSTRAINT FK_PostUsername FOREIGN KEY (username) REFERENCES \`BBY12-Users\` (username)
            );
            
            CREATE TABLE IF NOT EXISTS \`BBY12-PostImg\` (
                username VARCHAR(255) NOT NULL,
                postId INT NOT NULL,
                imgFile VARCHAR(255) NOT NULL UNIQUE,
                PRIMARY KEY (username , postid , imgFile),
                CONSTRAINT FK_ImgUsername FOREIGN KEY (username, postId) REFERENCES \`BBY12-Post\` (username, postId)
            );
                
            CREATE TABLE IF NOT EXISTS \`BBY12-PostTag\` (
                username VARCHAR(255) NOT NULL,
                postId INT NOT NULL,
                tag VARCHAR(255) NOT NULL UNIQUE,
                PRIMARY KEY (username , postid , tag),
                CONSTRAINT FK_TagUsername FOREIGN KEY (username, postId) REFERENCES \`BBY12-Post\` (username, postId)
            );
            
            CREATE TABLE IF NOT EXISTS \`BBY12-Admins\` (
                username VARCHAR(255) NOT NULL UNIQUE,
                PRIMARY KEY (username),
                CONSTRAINT FK_Admin FOREIGN KEY (username) REFERENCES \`BBY12-Users\` (username)
            );
            `);
